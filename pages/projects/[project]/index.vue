...
const postsLatestQuery = useLazyAsyncData(
  `project-${project}-latest-data`,
  () => queryContent(`projects/${project}`)
    // Selects only two kinds of paths:
    //   /projects/**/**
    //   /projects/**/**/index
    .where({ _path: new RegExp(`^/projects/${project}/[^\/]+$`) })
    .only(['_path', ...POST_MODEL_FIELDS])
    .sort({ modified: -1 })
    .find(),
)

const postCreatedQuery = useLazyAsyncData(
...
const postsCreatedIntoPostData = computed(() => postCreatedQuery.data.value?.map(intoPostData) ?? [])
...
<template>
  <div ml-a mr-a flex flex-col gap-10 xl:flex-row xl:gap-20>
    <div xl:sticky xl:top-0 xl:flex-self-start>
      <MarkdownReader xl:mt-22 />
    </div>
    <div
